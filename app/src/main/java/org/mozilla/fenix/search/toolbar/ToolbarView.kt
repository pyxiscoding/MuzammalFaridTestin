    ...
    init {
        view.apply {
            ...
            edit.colors = edit.colors.copy(
                text = context.getColorFromAttr(R.attr.blackWhiteTint),
                hint = context.getColorFromAttr(R.attr.blackWhiteTint),
                suggestionBackground = ContextCompat.getColor(
                    context,
                    R.color.suggestion_highlight_color
                ),
                clear = context.getColorFromAttr(R.attr.blackWhiteTint)
            )

            edit.setUrlBackground(
                AppCompatResources.getDrawable(context, R.drawable.home_search_background)
            )
    ...
    fun update(searchState: SearchFragmentState) {
        if (!isInitialized) {
            ...
            if (searchState.pastedText.isNullOrEmpty()) {
                ...
                /*
                * log alifhasnain
                * Updated url text on edit mode
                * */
                val updatedQuery = try {
                    val uri = Uri.parse(termOrQuery)
                    val parameter = uri.getQueryParameter("q")
                    when {
                        termOrQuery.startsWith("https://luxxle.com/luxsearch") -> {
                            parameter!!
                        }
                        termOrQuery.startsWith("https://luxxle.com/luximages") -> {
                            parameter!!
                        }
                        termOrQuery.startsWith("https://luxxle.com/luxvideos") -> {
                            parameter!!
                        }
                        termOrQuery.startsWith("https://luxxle.com/luxnews") -> {
                            parameter!!
                        }
                        else -> {
                            termOrQuery
                ...
                view.setSearchTerms(updatedQuery)
            }
        ...
        val d: Drawable = ContextCompat.getDrawable(context,R.drawable.ic_logo_home)!!
        val bitmap = d.toBitmap()
        val iconSize =
            context.resources.getDimensionPixelSize(R.dimen.preference_icon_drawable_size1)
