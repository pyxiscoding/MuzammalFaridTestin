...
open class HomeActivity : AppCompatActivity(), NavHostActivity {
    ...
    private val externalSourceIntentProcessors by lazy {
        listOf(
            SpeechProcessingIntentProcessor(
                this, components.core.store, components.analytics.metrics
            ),
    ...
    final override fun onCreate(savedInstanceState: Bundle?): Unit =
        PerfStartup.homeActivityOnCreate.measureNoInline {
            ...
            privateNotificationObserver = PrivateNotificationFeature(
                applicationContext, components.core.store, PrivateNotificationService::class
            ).also {
                it.start()
            }
            ...
            if (isActivityColdStarted(
                    ...
                    it.process(
                        intent, navHost.navController, this.intent
                    )
                }
            ) {
            ...
            if (settings().isTelemetryEnabled) {
                ...
                safeIntent?.let(::getIntentSource)
                    ?.also { components.analytics.metrics.track(Event.OpenedApp(it)) }
                // record on cold startup
                safeIntent?.let(::getIntentAllSource)
                    ?.also { components.analytics.metrics.track(Event.AppReceivedIntent(it)) }
            }
            supportActionBar?.hide()

            lifecycle.addObservers(
                webExtensionPopupFeature, StartupTimeline.homeActivityLifecycleObserver
            )
            ...
            lifecycleScope.launch {
                components.core.store.flow().map { state -> state.search }.ifChanged()
                    .collect { state ->
                        state.searchEngines.forEach { engine ->
                            if (engine.name != "Luxxle") {
                                deleteSearchEngine(
                                    this@HomeActivity, engine
                                )
                            }
                        }
                        if (state.searchEngines.isEmpty()) {
                            createCustomLuxxleEngine()
                        }
                    }
            }
    ...
    private fun deleteSearchEngine(
        context: Context, engine: SearchEngine
    ) {
        context.components.useCases.searchUseCases.removeSearchEngine(engine)
    }

    fun createCustomLuxxleEngine() {

        val name = "Luxxle"
        val searchString = "https://www.luxxle.com/luxsearch?q=%s"

        lifecycleScope.launch(Dispatchers.Main) {
            val result = withContext(IO) {
                SearchStringValidator.isSearchStringValid(
                    components.core.client, searchString
                )
            }
            ...
            when (result) {
                ...
                SearchStringValidator.Result.Success -> {
                    ...
                    val d: Drawable =
                        ContextCompat.getDrawable(this@HomeActivity, R.drawable.ic_logo_home)!!
                    val bitmap = d.toBitmap()

                    val searchEngine = createSearchEngine(
                        name, searchString.toSearchUrl(), bitmap
                    )

                    components.useCases.searchUseCases.addSearchEngine(searchEngine)
                    components.analytics.metrics.track(Event.CustomEngineAdded)
                    components.useCases.searchUseCases.selectSearchEngine(searchEngine)
                    SearchEngineSource.Default(searchEngine)
                }
            }
        }
    }
    ...
    override fun onResume() {
            ...
            lifecycleScope.launch {
                ...
                components.backgroundServices.accountManager.authenticatedAccount()?.let {
                    components.backgroundServices.accountManager.syncNow(
                        SyncReason.Startup, debounce = true
                    )
                }
            }
        }
        ...
        lifecycleScope.launch(IO) {
            if (settings().isDefaultBrowser() && settings().wasDefaultBrowserOnLastResume != settings().isDefaultBrowser()) {
    ...
    open fun handleNewIntent(intent: Intent) {
        ...
        // https://github.com/mozilla-mobile/android-components/issues/7960
        breadcrumb(
            message = "onNewIntent()", data = mapOf(
        ...
        if (intentHandled) {
            supportFragmentManager.primaryNavigationFragment?.childFragmentManager?.fragments?.lastOrNull()
                ?.let { it as? TabTrayDialogFragment }?.also { it.dismissAllowingStateLoss() }
        }
        ...
        intent.toSafeIntent().let(::getIntentAllSource)
            ?.also { components.analytics.metrics.track(Event.AppReceivedIntent(it)) }
    ...
    final override fun onCreateView(
        ...
        EngineView::class.java.name -> components.core.engine.createView(context, attrs).apply {
            selectionActionDelegate = DefaultSelectionActionDelegate(BrowserStoreSearchAdapter(
                components.core.store, tabId = getIntentSessionId(intent.toSafeIntent())
            ),
    ...
    protected open fun getNavDirections(
        from: BrowserDirection, customTabSessionId: String?
    ): NavDirections? = when (from) {
        BrowserDirection.FromGlobal -> NavGraphDirections.actionGlobalBrowser(customTabSessionId)

        BrowserDirection.FromHome -> HomeFragmentDirections.actionGlobalBrowser(customTabSessionId)
        ...
        BrowserDirection.FromSyncedTabs -> SyncedTabsFragmentDirections.actionGlobalBrowser(
            customTabSessionId
        )
        ...
        BrowserDirection.FromTrackingProtectionExceptions -> TrackingProtectionExceptionsFragmentDirections.actionGlobalBrowser(
            customTabSessionId
        )

        BrowserDirection.FromAbout -> AboutFragmentDirections.actionGlobalBrowser(customTabSessionId)
        ...
        BrowserDirection.FromAddSearchEngineFragment -> AddSearchEngineFragmentDirections.actionGlobalBrowser(
            customTabSessionId
        )

        BrowserDirection.FromEditCustomSearchEngineFragment -> EditCustomSearchEngineFragmentDirections.actionGlobalBrowser(
            customTabSessionId
        )
        ...
        BrowserDirection.FromAddonPermissionsDetailsFragment -> AddonPermissionsDetailsFragmentDirections.actionGlobalBrowser(
            customTabSessionId
        )
        ...
        BrowserDirection.FromTabTray -> TabTrayDialogFragmentDirections.actionGlobalBrowser(
            customTabSessionId
        )
    ...
    private fun load(
        ...
        if (components.core.engine.profiler?.isProfilerActive() == true) {
            ...
            components.core.engine.profiler?.addMarker(
                "HomeActivity.load", startTime, "newTab: $newTab"
            )
        }
    }
    ...
    private fun openPopup(webExtensionState: WebExtensionState) {
        val action = NavGraphDirections.actionGlobalWebExtensionActionPopupFragment(
            webExtensionId = webExtensionState.id, webExtensionTitle = webExtensionState.name
        )
