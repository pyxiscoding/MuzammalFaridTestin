...
sealed class Event {
    ...
    data class BrowserMenuItemTapped(val item: Item) : Event() {
        enum class Item {
            ...
            BOOKMARKS, HISTORY, SYNC_TABS, DOWNLOADS, DELETE_BROWSING_DATA,SEARCH_PREFERENCE
        }

        override val extras: Map<Events.browserMenuActionKeys, String>?
            get() = mapOf(Events.browserMenuActionKeys.item to item.toString()
    ...
    data class TabCounterMenuItemTapped(val item: Item) : Event() {
        ...
        override val extras: Map<Events.tabCounterMenuActionKeys, String>?
            get() = mapOf(Events.tabCounterMenuActionKeys.item to item.toString()
                .lowercase(Locale.ROOT))
    }
    ...
    data class AutoPlaySettingChanged(val setting: AutoplaySetting) : Event() {
        ...
        override val extras: Map<Autoplay.settingChangedKeys, String>?
            get() = mapOf(Autoplay.settingChangedKeys.autoplaySetting to setting.toString()
                .lowercase(Locale.ROOT))
    }
