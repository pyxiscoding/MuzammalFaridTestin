...
class SelectBookmarkFolderAdapter(private val sharedViewModel: BookmarksSharedViewModel) :
    ...
    fun updateData(tree: BookmarkNode?, hideFolderGuid: String?) {
        ...
        /*
        * log alifhasnain
        * Removed specific bookmarks while adding bookmark
        * */
        updatedData.removeIf { it.node.guid == "menu________" }
        updatedData.removeIf { it.node.guid == "toolbar_____" }
        updatedData.removeIf { it.node.guid == "unfiled_____" }
