package org.mozilla.fenix.moduleclasses.issueone
...
class WebAppShortcutManager(
    ...
    suspend fun requestPinShortcut(
        context: Context, session: SessionState, overrideShortcutName: String? = null
    ) {
    ...
    suspend fun updateShortcuts(context: Context, manifests: List<WebAppManifest>) {
        if (SDK_INT >= VERSION_CODES.R) {
            context.getSystemService<ShortcutManager>()?.apply {
                val shortcuts =
                    manifests.mapNotNull { buildWebAppShortcut(context, it)?.toShortcutInfo() }
    ...
    suspend fun buildBasicShortcut(
        ...
        val shortLabel =
            overrideShortcutName ?: manifest?.shortName ?: manifest?.name ?: session.content.title

        val fallback = fallbackLabel
        val fixedLabel = shortLabel.ifBlank { fallback }

        val builder =
            ShortcutInfoCompat.Builder(context, session.content.url).setShortLabel(fixedLabel)
    ...
    suspend fun buildWebAppShortcut(
        context: Context, manifest: WebAppManifest
    ): ShortcutInfoCompat? {
        ...
        return ShortcutInfoCompat.Builder(context, manifest.startUrl).setLongLabel(manifest.name)
            .setShortLabel(shortLabel.ifBlank { fallbackLabel })
            .setIcon(buildIconFromManifest(manifest)).setIntent(shortcutIntent).build()
    }
     ...
     */
    fun findShortcut(context: Context, startUrl: String) = if (SDK_INT >= VERSION_CODES.R) {
    ...
    suspend fun getWebAppInstallState(
        url: String, @VisibleForTesting currentTimeMs: Long = System.currentTimeMillis()
    ): WebAppInstallState {
...
private fun webAppIcons(
    ...
    loaders = listOf(
        MemoryIconLoader(pwaIconMemoryCache), HttpIconLoader(httpClient), DataUriIconLoader()
    ),
    decoders = listOf(
        AndroidImageDecoder(), ICOIconDecoder()
    ),
    processors = listOf(
